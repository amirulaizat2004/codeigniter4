...
<?= csrf_field() ?>
  <div class="form-group row">
    ...
    <div class="col-sm-10">
      <input type="text" name="student_name" class="form-control <?= ($validation->getError('student_name'))?'is-invalid':''  ?>" id="student_name" value="<?= old('student_name') ?>">
  ...
  <div class="form-group row">
    ...
    <div class="col-sm-10">
      <input type="text" name="matrik_number" class="form-control <?= ($validation->getError('matrik_number'))?'is-invalid':''  ?>" id="matrik_number" value="<?= old('matrik_number') ?>">
      <div class="invalid-feedback">
  ...
  <div class="form-group row">
    ...
    <select id="state" name="state" class="form-control <?= ($validation->getError('state'))?'is-invalid':''  ?>">
        ...
        <?php foreach ($state as $key => $value) { ?>
          <option <?=(old('state')==$key)?'selected':''  ?> value="<?= $key ?>"><?= $value ?></option>
        <?php } ?>
      </select>
      <div class="invalid-feedback">
  ...
  <div class="form-group row">
    ...
    <select id="inputState" name="id_course1" class="form-control <?= ($validation->getError('id_course1'))?'is-invalid':''  ?>">
        ...
        <?php foreach ($course as $key => $value) { ?>
          <option <?=(old('id_course1')==$key)?'selected':''  ?> value="<?= $key ?>"><?= $value ?></option>
        <?php } ?>
  ...
  <div class="form-group row">
    ...
    <select id="id_course2" name="id_course2" class="form-control <?= ($validation->getError('id_course2'))?'is-invalid':''  ?>">
        <option value="">Pilih...</option>
        <?php foreach ($course as $key => $value) { ?>
          <option <?=(old('id_course2')==$key)?'selected':''  ?> value="<?= $key ?>"><?= $value ?></option>
        <?php } ?>
    ...
    <div class="row">
      ...
      <div class="col-sm-10">
          ...
          <?php foreach ($gender as $key => $value) { ?>
            ...
            <input name="gender" class="form-check-input <?= ($validation->getError('gender'))?'is-invalid':''  ?>" type="radio" name="gridRadios" id="gridRadios1" <?=(old('gender')==$key)?'checked':''  ?> value="<?= $key ?>">
